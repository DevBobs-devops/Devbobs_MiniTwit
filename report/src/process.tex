...
the MiniTwit application for Windows, Mac and Linux and zips the source code, including this report.
In hindsight our releases are not perfect.
We automatically increased the version of each release which is why we ended up with version \textttt{0.0.259}, having no major or minor versions.
The releases did not contain any description, which creates bad transparency.
...
We use Alloy to collect logs from each node and forward them to Loki. The logs are displayed on grafana, which can
be seen in the logging dashboard (figure~\ref{logging-dashboard}).
