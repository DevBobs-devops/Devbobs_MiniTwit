...
living on the ``Leader'' node, that takes HTTPS requests on our \code{devbobs.tech} domain, 
means that if the ``Leader'' fails,  HTTPS request would no longer be recieved by the system. 
If the ``Leader'' node goes down, it is still possible to access the other nodes
