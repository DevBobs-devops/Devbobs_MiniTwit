...
\textit{Morten}
SonarQube runs on pull requests, checking for errors in the source code. SonarQube is not blocking 
...
All nodes run Alloy.
The "Leader" node also runs Caddy, Grafana, Prometheus and Loki. See
...
As we only have one Caddy loadbalancer service,
...
will in that case take care of loadbalancing, instead of Caddy.
