...
The first workflow ("\textit{Build and Test}"), forms the continous intergration of our pipeline and consists of the following steps that run in parrallel
\begin{enumerate}
...
Lastly, instead of ziping our Minitwit application, we could have used our docker images to encapsulate each part of the system in releases.
...
The system uses a reverse proxy, which takes HTTPS request and decrypts them.
This is done with Caddy, as was seen in figure \ref{deployment-diagram}.
In reflection more of the system, and the information it exposes, should have been hidden behind the reverse proxy and had firewalls, as many ports can still be reached with HTTP.
