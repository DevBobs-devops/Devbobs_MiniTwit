...
\code{ubuntu-24.04}. A complete illustration can be seen HERE (IMAGE).
...
\begin{enumerate}
    ...
    \item \textbf{Build the application}: Builds the applications and runs test:
    \begin{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Builds and pushes the \code{Minitwit}, \code{Grafana}, \code{Alloy}, \code{Loki} and \code{Prometheus} docker images to Dockerhub.
    \end{itemize}
    \item \textbf{SSH and Deploy}
    \begin{itemize}
        \item SSH into the node named leader in the swarm.
        \item Copy the stack file \code{/itu-minitwit/infrastructure/docker\_swarm/stack/minitwit\_stack.yml} to the node.
...
Monitoring is done on a business level, application level, and
...
\begin{itemize}
    ...
    \item Average
    \item follower count per user
    \item Total cheeps over time
\end{itemize}
...
On a application level we monitor different database queries that
MiniTwit makes:
\begin{itemize}
    \item GetCheeps query durations
    \item GetCheepsFromAuthor
    \item AddCheep
\end{itemize}
...
\begin{itemize}
    ...
    \item HTTP per minutte for each node
\end{itemize}
...
Logs from each docker service in
\code{/infrastructure/docker\_swarm/stack/minitwit\_stack.yml} are
logged. \emph{In reflection more should be logged?}
(BURDE VI HAVE MERE???)
...
cluster, that together run four replicated minitwit services, see
\code{terraform/minitwit\_swarm\_cluster.tf} and
\code{docker\_swarm/stack/minitwit\_stack.yml} under
\code{/infrastructure}. This ensures that even if a service or node
fails, the application is still available

\textbf{SPOF}: We currently only have one Caddy loadbalancer service,
living on the leader node, that takes https requests on our
\code{devbobs.tech} domain. This is a single point of failure for
accessing the application from \code{devbobs.tech} and with https. If
the leader goes down, it is still possible to access the other nodes
ip's on port :8080 without https, where the docker ingress routing mesh
will take care of loadbalancing.
...
node and then make it join the swarm, by running the \code{bootstrap.sh} script again.
