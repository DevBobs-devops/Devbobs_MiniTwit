...
public class FollowRepository : IFollowRepository
{
    ...
    {
        var follow = _context.Follows.FirstOrDefault(follow =>
            follow.Follower == followerName && follow.Followed == followedName
        );
        if (follow != null) //we must ensure it is not already in the database
        {
            return;
        }
        var newFollow = new Follow() { Follower = followerName, Followed = followedName };

        await _context.Follows.AddAsync(newFollow);
        await _context.SaveChangesAsync();
        FollowMetrics.IncrementFollower(followedName);
    }
    ...
    public async Task RemoveFollowing(string followerName, string followedName)
    {
        ...
        if (follow != null)
        {
            ...
            await _context.SaveChangesAsync();
            FollowMetrics.DecrementFollower(followedName);
        }
    }
