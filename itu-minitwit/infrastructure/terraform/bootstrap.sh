...
echo -e "As per the README, to setup CI/CD pipeline, add the following to your github secrets:"
echo -e " - CONNECTION_STRING: $(terraform output -raw ConnectionStrings__DefaultConnection)"
echo -e " - DOCKER_USERNAME:   <your docker hub username>"
echo -e " - DOCKER_PASSWORD:   <your docker hub password/token> - see README"
echo -e " - SSH_KEY:           $(cat ssh_key/terraform)"
echo -e " - SSH_USER:          root"
echo -e " - SSH_HOST:          $(terraform output -raw public_ip)"
echo -e " ------------------------------------------------------ "
